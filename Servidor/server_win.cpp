...
namespace Servidor {
    ...
    namespace Controller {
        ...
        public:
            ...
            void encaminharMensagem(const std::string &remetente, const std::string &destinatarioUsername,
                                    const std::string &conteudo);

            void encaminharNotificacaoDigitando(const std::string &remetente, const std::string &destinatarioUsername,
                                                bool estaDigitando);

            void broadcastStatusUpdate(uint32_t pIdExcluido, const std::string &username, bool isOnline);

            void salvarMensagemOffline(uint32_t destinatarioId, const std::string &remetente,
                                       const std::string &conteudo, time_t timestamp) {
                ...
                if (arquivo.is_open()) {
                    ...
                    std::cerr << "[ERRO] Nao foi possivel salvar mensagem offline para o usuario ID " << destinatarioId
                            << std::endl;
                }
            }
        };

        TratadorCliente::TratadorCliente(SOCKET socket, ChatServidor *servidor,
                                         Persistencia::GerenciadorUsuarios *gerenciador)
            : socketCliente(socket), instanciaServidor(servidor), gerenciadorUsuarios(gerenciador),
        ...
        void ChatServidor::encaminharMensagem(const std::string &remetente, const std::string &destinatarioUsername,
                                              const std::string &conteudo) {
            auto destinatarioUserObj = gerenciadorUsuarios.findUserByUsername(destinatarioUsername);
            if (!destinatarioUserObj) {
                std::cout << "[AVISO] Tentativa de enviar mensagem para usuario inexistente: " << destinatarioUsername
                        << std::endl;
                return;
            }
            ...
            if (it != sessoesAtivas.end()) {
                ...
                std::cout << "[MSG] Mensagem de '" << remetente << "' para '" << destinatarioUsername <<
                        "' encaminhada com sucesso." << std::endl;
            } else {
                std::cout << "[MSG] Usuario '" << destinatarioUsername << "' esta offline. Salvando mensagem..." <<
                        std::endl;
                time_t timestamp = time(0);
        ...
        void ChatServidor::encaminharNotificacaoDigitando(const std::string &remetente,
                                                          const std::string &destinatarioUsername, bool estaDigitando) {
            auto destinatarioUserObj = gerenciadorUsuarios.findUserByUsername(destinatarioUsername);
